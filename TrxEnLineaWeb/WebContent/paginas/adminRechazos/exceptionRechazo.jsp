...
<html:form method="post" action="/init_discount_rechazos"
	...
	<logic:present name="dynamicELForm" property="el_string">
		<bean:size id="size" name="dynamicELForm" property="el_string" />
		...
		<logic:greaterThan value="0" name="size">
			<table>
				<logic:present name="parametros_strings" scope="application">
					...
					Hashtable hashCampos_ = (Hashtable) hash_.get("tx_discount");
					...
					<logic:present name="list_campos" scope="request">
						...
						<logic:iterate id="element" name="list_campos">
							<tr>
								...
								<logic:iterate id="el_string" name="dynamicELForm"
									property="el_string" indexId="i"
									type="com.asic.transacciones.vo.TX_EXCEPTION_LOG_Imp">
									<td class="EnlacesMenuSubSmall" align="center"><html:text
										name="el_string" indexed="true"
										property='<%=("" + element).toUpperCase()%>'>
										<bean:write name="el_string"
											property='<%=("" + element).toUpperCase()%>' />
									</html:text></td>
								</logic:iterate>
							</tr>
